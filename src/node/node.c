...
static void node_state_compute_update(node_t *node);
static void node_state_wait_for_server(node_t *node);
static void node_state_wait_final(node_t *node);
...
void node_set_update_data(node_t* node, const update_t* data, size_t len){
    if(len > UPDATE_LEN) len = UPDATE_LEN;
    memcpy(node->data_update, data, len * sizeof(update_t));
}

void run_node_state(node_t* node){
    if(node && node->current_state){
...
void node_state_compute_update(node_t *node){
    ...
    if(node->deps.io.send(node->deps.io.obj, (uint8_t*)&local_update_msg, sizeof(local_update_msg)) == OK){
        #if DEBUG
...
void node_state_wait_for_server(node_t *node){
    ...
    if(node->deps.io.recv(node->deps.io.obj, (uint8_t*)&srv_dropout_msg, sizeof(srv_dropout_list_t), &out_len) != OK){
        #if DEBUG
    ...
    node->deps.io.send(node->deps.io.obj, (uint8_t*)&share_rec_msg, sizeof(share_rec_msg));
...
void node_state_wait_final(node_t *node){
    ...
    if(node->deps.io.recv(node->deps.io.obj, (uint8_t*)&final_msg, sizeof(srv_global_update_t), &out_len) != OK){
        #if DEBUG
